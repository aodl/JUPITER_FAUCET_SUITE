...
fn main() {
    println!("cargo:rerun-if-env-changed=JUPITER_FAUCET_PROD_CANISTER_ID");
    ...
    let canonical_prod_id = load_ic_canister_id(&canister_ids_path, "jupiter_faucet");

    if let Ok(value) = env::var("JUPITER_FAUCET_PROD_CANISTER_ID") {
    ...
    println!("cargo:rustc-env=JUPITER_FAUCET_PROD_CANISTER_ID={}", canonical_prod_id);
}
