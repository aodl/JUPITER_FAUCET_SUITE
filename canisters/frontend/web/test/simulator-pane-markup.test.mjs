...
test('How it works pane includes advanced usage memo builder without restoring simulator controls', () => {
  ...
  assert.match(howItWorks, /id="memo-builder-tip-url"[^>]*data-copy-value="#how-it-works:2\?canister=\{protocol canister ID\}&amp;title=\{custom title\}&amp;label=\{custom label\}"/);
...
test('metric tracker hash deep links submit once on cold load and panel open', () => {
  ...
  assert.match(mainJs, /submit && lastHashSubmitMemo !== `\$\{memoText\}\|\$\{state\.protocolCanisterText\}\|\$\{state\.range\}`/);
  assert.match(mainJs, /lastHashSubmitMemo = `\$\{memoText\}\|\$\{state\.protocolCanisterText\}\|\$\{state\.range\}`/);
