...
Relay Setup presents reusable repeatable controls for 1–20 target canisters and an explicit surplus mode: zero recipients for all-cycles allocation, or 1–5 typed recipient rows. Routing mode requires at least one complete row even when submission is invoked programmatically; all-cycles mode submits exactly the empty vector and ignores hidden editor values. Every row independently selects Principal or Neuron ID and a Text or Hexadecimal memo. Text is preserved exactly and counted as UTF-8 bytes; Hexadecimal is the authoritative representation for arbitrary bytes. A mode switch is accepted only after assigning the candidate representation to the real input and proving an exact byte-for-byte round trip. If browser input handling or UTF-8 decoding would change the bytes, the original Hexadecimal string and mode are restored and a non-blocking conversion notice is announced without invalidating the configuration. Empty means no outgoing memo and the maximum is 32 bytes, matching `jupiter_memo_policy::MAX_RELAY_SURPLUS_MEMO_BYTES`. Malformed, destination-duplicate, and over-limit values are rejected before calling Historian; memos never make a duplicate destination legal. Neuron IDs use exact nonzero `u64` parsing with `BigInt`. All-cycles and Principal-only configurations make no Governance request. For a new configuration containing neurons, the browser confirms each unique neuron is public/readable through NNS Governance before exposing payment details. Historian remains authoritative for canonical order and account, reserved canonical-target rejection, protected-target policy, and recipient policy.

The returned view displays both canonical orders, recipient destination and mandatory exact lowercase memo hex, both counts, all-cycles mode where applicable, the configuration hash, and nominal target-count pricing. Optional UTF-8 text is shown only after an exact round trip and only when it is non-whitespace and contains no Unicode control, format, surrogate, private-use, unassigned, bidirectional-control, line-separator, or paragraph-separator character. Rendering uses `textContent`; this display rule never modifies or rejects the underlying bytes. For a new available configuration, the browser reads its informational balance directly from the ICP ledger with `icrc1_balance_of`; it never scans the index or submits a transaction reference. The browser polls while that setup account is visible, but funding never triggers creation automatically and Governance preflight is not repeated during polling. The **Create Relay** button sends both vectors to `notify_relay_configuration`. One authoritative fingerprint contains surplus mode and every raw target, recipient type, destination, memo mode, and memo input. Create rechecks the current raw form fingerprint against the authoritative checked view immediately before notification; a mismatch clears the stale account and requires another check. Any memo interaction, edit, or surplus-mode change clears the displayed account and prevents stale setup-view, neuron lookup, or notify responses from mutating the new configuration. The browser never derives or guesses a setup account locally.

Setup deposits are aggregate protocol deposits. They are not attributed to individual payers and are not automatically refundable. Targets, typed destinations, and exact memo bytes jointly determine the setup address through one canonical encoding and are immutable after creation. Principals receive at their default ICP accounts; public NNS neurons receive at their resolved Governance staking accounts, and Relay requests `claim_or_refresh` after each successful neuron transfer. With recipients, surplus is divided equally after top-up and safety gates. With zero recipients, all-cycles mode allocates fee-efficient funding among measured positive cycles needs and routes no raw ICP surplus. No IO recipient is injected automatically. Creation pricing remains target-based.

Only an authoritative `Active` configuration exposes operational funding guidance. This covers an existing Active configuration after reload and an Active notification received before the next query has caught up; `InProgress`, `ManualRecoveryRequired`, and every other state remain setup/recovery views. The browser derives all funding accounts locally from that verified child principal and clears them whenever the edited configuration invalidates the authoritative view. It never queries the finalized controllerless Relay for addresses.

The Active view shows Relay's default account, staging subaccount 1, and splitter subaccounts 10–90. Each card includes the ICRC account text, legacy ICP account identifier, explicit owner, and full 32-byte subaccount. It also links to the same application's memo builder in explicit raw-ICP mode for the Active Relay's default account. The link only constructs the route memo: the donor still sends the endowment ICP to the Faucet neuron's staking account, and later Faucet payouts arrive at the Relay default account. Funding-account selection—not a transaction memo containing a number—selects a splitter, and unsupported Relay subaccounts are not promised to be swept or recoverable.
