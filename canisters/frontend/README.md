...
Relay Setup presents reusable repeatable controls for 1–20 target canisters and 1–5 typed surplus recipients. Every recipient row independently selects Principal or Neuron ID and defaults to Principal. Empty, malformed, typed-duplicate, and over-limit lists are rejected before calling Historian; neuron IDs use exact nonzero `u64` parsing with `BigInt`. Principal-only configurations make no Governance request. The browser first asks Historian for the authoritative configuration view and immediately renders existing Active, InProgress, or ManualRecoveryRequired configurations without a Governance preflight. For a new configuration containing neurons, it holds the returned view locally and uses the same `HttpAgent` to confirm each unique neuron is public/readable through NNS Governance before storing or rendering the view or exposing payment details. Historian remains authoritative for canonical order, reserved canonical-target rejection, protected-target policy, and recipient policy.

The returned view displays both canonical orders with recipient type labels, both counts, the configuration hash, and nominal target-count pricing. For a new available configuration, the browser reads its informational balance directly from the ICP ledger with `icrc1_balance_of`; it never scans the index or submits a transaction reference. The browser polls while that setup account is visible, but funding never triggers creation automatically and Governance preflight is not repeated during polling. The **Create Relay** button sends both vectors to `notify_relay_configuration`. One authoritative fingerprint contains every raw input and recipient type, so changing either category stops polling, clears account and notification state, and prevents stale query, neuron lookup, or notify responses from mutating the new configuration. If notification returns a higher live requirement, the browser retains it until funding is sufficient for a later explicit attempt.

Setup deposits are aggregate protocol deposits. They are not attributed to individual payers and are not automatically refundable. Targets and typed recipients jointly determine the setup address, are immutable after creation, and return the existing Relay only for the same full configuration. Principals receive at their default ICP accounts; public NNS neurons receive at their resolved Governance staking accounts, and Relay requests `claim_or_refresh` after each successful neuron transfer. Surplus is divided equally after top-up and safety gates; each net share must reach 1 ICP, and no IO recipient is injected automatically. Creation pricing remains target-based.
