...
The configured production ICP Index contract is newest-first. A persisted legacy ascending-order flag is unsupported because its historical coverage cannot be proved safely. Staking indexing fails closed before an outcall, marks route totals incomplete, and latches an actionable fault. Output or rewards indexing likewise fails closed during ordinary runtime rather than reinterpreting an ascending cursor. A one-hop `post_upgrade` repair handles only an affected output or rewards marker that is exactly `Some(false)`: it logs the discarded total/cursors, resets only that derived route aggregate and continuation, changes the marker to `Some(true)`, and lets the current bounded newest-first indexer rebuild from authoritative ICP Ledger/Index history. It does not reset staking/endowment state, histories, tracking, cycles/SNS/XRC state, Relay setup memory 26, or lifetime route rollups in memory 29. `get_public_status().route_index_fault` remains non-null while either historical route backfill is incomplete, so temporarily lower or partial output/rewards totals are explicitly degraded until genesis coverage is established. The rebuilt total is authoritative even if it differs from the legacy aggregate. A genuine staking fault remains visible until an operator establishes that the invariant is resolved and explicitly clears it; a superficially successful endowment refresh does not clear it merely to return success.
