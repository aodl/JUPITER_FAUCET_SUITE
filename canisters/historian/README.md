...
Healthy production requires the commitment-route health query to report `complete_from_genesis = true` and `commitment_index_fault = null`, and `get_public_status()` to report `route_index_fault = null`, before and after a routine upgrade. The public queries must already fail closed immediately after post-upgrade restore if persisted index ordering is unsupported. After Historian is started, allow the normal initial or forced main tick to run before asserting that staking/endowment and output/rewards indexing have resumed normally. `qualifying_commitment_count` must not decrease across the upgrade; an increase after restart/indexing is permitted and may represent newly indexed qualifying endowments. Bounded retained histories must never be used to reconstruct or reduce the lifetime count. If completeness unexpectedly becomes false or either index fault is present, investigate the invariant violation rather than initiating a historical rebuild. Existing production Historian must be upgraded in place and must not be reinstalled.
