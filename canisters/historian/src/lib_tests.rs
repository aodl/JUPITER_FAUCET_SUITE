...
mod tests {
    ...
    fn seed_legacy_route_state(output_order: Option<bool>, rewards_order: Option<bool>) -> State {
        let mut st = base_state();
        st.total_output_e8s = Some(111);
        st.last_indexed_output_tx_id = Some(110);
        st.oldest_indexed_output_tx_id = Some(11);
        st.output_route_index_descending = output_order;
        st.output_route_backfill_complete = Some(true);
        st.active_output_catch_up = Some(DescendingIndexCatchUp {
            boundary_tx_id: 110,
            observed_head_tx_id: 120,
            next_start_tx_id: Some(115),
        });
        st.total_rewards_e8s = Some(222);
        st.last_indexed_rewards_tx_id = Some(220);
        st.oldest_indexed_rewards_tx_id = Some(22);
        st.rewards_route_index_descending = rewards_order;
        st.rewards_route_backfill_complete = Some(true);
        st.active_rewards_catch_up = Some(DescendingIndexCatchUp {
            boundary_tx_id: 220,
            observed_head_tx_id: 230,
            next_start_tx_id: Some(225),
        });
        st.active_route_sweep = Some(ActiveRouteSweep {
            started_at_ts_nanos: 999,
            next_index: 1,
        });
        st.last_completed_route_sweep_ts = Some(998);
        st
    }
    ...
    struct RouteStateSnapshot {
        ...
        output_latest: Option<u64>,
        output_oldest: Option<u64>,
        output_descending: Option<bool>,
        output_complete: Option<bool>,
        output_catch_up: Option<DescendingIndexCatchUp>,
        rewards_total: Option<u64>,
        rewards_latest: Option<u64>,
        rewards_oldest: Option<u64>,
        rewards_descending: Option<bool>,
        rewards_complete: Option<bool>,
        rewards_catch_up: Option<DescendingIndexCatchUp>,
        active_sweep: Option<ActiveRouteSweep>,
        last_completed_sweep_ts: Option<u64>,
    }

    fn route_state_snapshot(st: &State) -> RouteStateSnapshot {
        RouteStateSnapshot {
            output_total: st.total_output_e8s,
            output_latest: st.last_indexed_output_tx_id,
            output_oldest: st.oldest_indexed_output_tx_id,
            output_descending: st.output_route_index_descending,
            output_complete: st.output_route_backfill_complete,
            output_catch_up: st.active_output_catch_up.clone(),
            rewards_total: st.total_rewards_e8s,
            rewards_latest: st.last_indexed_rewards_tx_id,
            rewards_oldest: st.oldest_indexed_rewards_tx_id,
            rewards_descending: st.rewards_route_index_descending,
            rewards_complete: st.rewards_route_backfill_complete,
            rewards_catch_up: st.active_rewards_catch_up.clone(),
            active_sweep: st.active_route_sweep.clone(),
            last_completed_sweep_ts: st.last_completed_route_sweep_ts,
        }
    }

    #[test]
    fn post_upgrade_route_repair_resets_both_legacy_routes_and_preserves_unrelated_state() {
        let sentinel = principal("jufzc-caaaa-aaaar-qb5da-cai");
        let mut st = seed_legacy_route_state(Some(false), Some(false));
        st.last_indexed_staking_tx_id = Some(333);
        st.oldest_indexed_staking_tx_id = Some(3);
        ...
        st.qualifying_commitment_count = Some(44);
        st.initial_cycles_probe_queue = vec![sentinel];
        st.last_sns_discovery_ts = 55;
        let config_before = runtime_config_log_line(&st.config);
        ...
        assert_eq!(runtime_config_log_line(&st.config), config_before);
        assert_eq!(st.last_indexed_staking_tx_id, Some(333));
        assert_eq!(st.oldest_indexed_staking_tx_id, Some(3));
        ...
        assert_eq!(st.qualifying_commitment_count, Some(44));
        assert_eq!(st.initial_cycles_probe_queue, vec![sentinel]);
        assert_eq!(st.last_sns_discovery_ts, 55);
    }

    #[test]
    fn post_upgrade_route_repair_resets_only_output_when_only_output_is_legacy() {
        let mut st = seed_legacy_route_state(Some(false), Some(true));
        let rewards_before = (
            st.total_rewards_e8s,
            st.last_indexed_rewards_tx_id,
            st.oldest_indexed_rewards_tx_id,
            st.rewards_route_index_descending,
            st.rewards_route_backfill_complete,
            st.active_rewards_catch_up.clone(),
        );
    ...
    #[test]
    fn post_upgrade_route_repair_resets_only_rewards_when_only_rewards_is_legacy() {
        let mut st = seed_legacy_route_state(Some(true), Some(false));
        let output_before = (
            st.total_output_e8s,
            ...
            st.active_output_catch_up.clone(),
        );
    ...
    #[test]
    fn post_upgrade_route_repair_leaves_current_complete_state_unchanged() {
        let mut st = seed_legacy_route_state(Some(true), Some(true));
        let before = route_state_snapshot(&st);

        repair_legacy_ascending_route_indexes_after_upgrade(&mut st);

        assert_eq!(route_state_snapshot(&st), before);
    }

    #[test]
    fn post_upgrade_route_repair_is_idempotent_during_current_backfill() {
        let mut st = seed_legacy_route_state(Some(false), Some(true));
        repair_legacy_ascending_route_indexes_after_upgrade(&mut st);
        st.total_output_e8s = Some(77);
        st.last_indexed_output_tx_id = Some(700);
        st.oldest_indexed_output_tx_id = Some(650);
        st.active_output_catch_up = Some(DescendingIndexCatchUp {
            boundary_tx_id: 700,
            observed_head_tx_id: 710,
            next_start_tx_id: Some(675),
        });
        let before = route_state_snapshot(&st);
    ...
    #[test]
    fn route_health_reports_unsupported_and_incomplete_historical_coverage() {
        let mut st = base_state();
        st.output_route_index_descending = Some(false);
        st.rewards_route_index_descending = Some(false);
        assert_eq!(
            route_index_fault(&st).as_deref(),
            Some("unsupported persisted ascending output and rewards pagination state")
        );
        ...
        assert_eq!(
            route_index_fault(&st).as_deref(),
            Some("output historical route backfill is incomplete")
        );

        st.output_route_backfill_complete = Some(true);
        assert!(route_index_fault(&st).is_none());
    }
