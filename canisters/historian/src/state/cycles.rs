...
pub(super) fn sync_all_cycles_history_maps(current: &BTreeMap<Principal, Vec<CyclesSample>>) {
    ...
    for (principal, samples) in current {
        let timestamps: Vec<u64> = samples
            .iter()
        ...
        if !timestamps.is_empty() {
            ...
            with_cycles_entry_map(|map| {
                for sample in samples {
                    map.insert(
                        CyclesEntryKey::new(principal, sample.timestamp_nanos),
                        sample.clone(),
                    );
                }
            });
...
pub(super) fn sync_cycles_history_principals(
    ...
    for principal in principals {
        ...
        let current_timestamps: Vec<u64> = current_samples
            .iter()
            .map(|sample| sample.timestamp_nanos)
            .collect();
...
pub(super) fn sync_all_raw_icp_commitment_history_maps(
...
pub(super) fn sync_all_neuron_commitment_history_maps(
    ...
    for (neuron_id, samples) in current {
        ...
        if !ids.is_empty() {
            ...
            with_neuron_commitment_entry_map(|map| {
                for sample in samples {
                    map.insert(
                        NeuronCommitmentEntryKey::new(*neuron_id, sample.tx_id),
                        sample.clone(),
                    );
                }
            });
