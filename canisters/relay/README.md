...
There are two versioned stable journals using non-overlapping stable-memory IDs. Memory 0 contains SNS reward attribution state: epoch Root, processed Faucet-commitment cursor and carry, the nine bounded splitter attribution cursors and carries, last weekly sweep attempt, and any fully pinned pending reward-token transfer with its proposed splitter updates. Its explicit V2 schema migrates the frozen prior V1 wire shape once, preserving every old transfer-identity field and initializing splitter boundaries and old pending proposals empty. Memory 1 separately stores splitter execution transactions, driver fencing revisions, and quarantine evidence; reward attribution never derives provenance from that mutable journal. Active splitter work survives a same-ledger upgrade, while an upgrade that changes the configured ICP Ledger during an active split fails closed. A reinstall clears both stable memories with the canister's stable memory.
