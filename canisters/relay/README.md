...
- **Current and future funding can be combined.** Relay can keep some ICP spendable now while routing another portion into a Jupiter Faucet endowment that feeds the Relay again in future.
...
- **subaccount 1:** ICP Relay will use to create a Jupiter Faucet endowment that can feed its default account again in future.

The numbered splitter subaccounts divide one deposit between those buckets.

| Funding path | Immediate liquidity | Builds Faucet endowment | Best fit |
|---|---:|---:|---|
| Relay default account | 100% | 0% | Bootstrap, normal direct funding, recovery |
| Direct Jupiter Faucet endowment targeting Relay | Future Faucet payouts | 100% | Cleanest perpetual funding path |
...
That creates a normal Faucet endowment targeting the Relay; future payouts return to the Relay's default account. Subaccount 1 is useful when the original sender cannot attach the Faucet memo or when many small deposits need to accumulate before they qualify.
...
Reward attribution is a separate stage; it does not change the cycles-allocation rules. Relay normally adjudicates once per week, at the start of an accepted daily main tick, by asking the configured [`jupiter-sns-rewards`](../sns-rewards/README.md) canister for a fresh completed owner snapshot and its Root-resolved token Ledger. A completed adjudication, including one with boundedly abandoned recipients, starts the next seven-day interval. A dependency, history, or reconstruction failure before valid adjudication does not, so Relay retries at the next accepted daily main tick. A small durable payout cursor protects exact transfer identities across awaits and upgrades, but always consumes itself rather than becoming a recovery journal.

Relay distributes its available economical SNS reward-token balance to identifiable funders of the most recent completed Jupiter Faucet endowment containing at least one eligible funder. Eligible funders share the distributable reward pro rata according to the ICP they contributed to that endowment. Unknown, minted, mismatched, and other ineligible ICP has zero reward weight: it receives nothing, cannot veto or dilute eligible recipients, and remains visible as one aggregate. If newer completed endowments contain no eligible funder, Relay walks backwards to the most recent one that does. If genuine history exhaustion proves that none exists, Relay leaves the token pot untouched.

Relay attributes its current reward-token pot only to Faucet endowments that predate both the active SNS owner snapshot cutoff and the oldest unspent reward-token credit currently making up that pot. Relay reconstructs the current reward balance statelessly from the Root-resolved SNS Index. This prevents a later ICP endowment from capturing SNS tokens that were already present before that endowment existed. Completed endowments are not marked as rewarded, so the same endowment can receive later SNS-token accruals until a newer qualifying endowment predates the applicable reward pot.
...
If the current balance contains reward credits from several times, Relay uses the oldest unspent non-zero credit as the cutoff for the whole pot. This may favour an older valid endowment over a newer one, but it never intentionally moves an older reward accrual forward to a later contributor. Once the old pot is spent, later residual credits retain their own Ledger timestamps and may naturally select a newer endowment. Detectable Reward Index lag, including any live/index balance mismatch, fails closed without consuming the weekly cadence, as do Root/component mismatch, malformed accounting, and pagination corruption. An unindexed net-zero suffix can leave the Index and Ledger balances equal; in that case the stale history can only conservatively bias the cutoff older than the fully synchronized cutoff. An already-pinned payout uses its adjudicated identities and does not recalculate the cutoff.
...
Before its first token transfer, Relay durably pins the token Ledger, Faucet endowment ID, fee, deterministic recipients, exact transfer identities and current recipient index. Success or Ledger `Duplicate` advances exactly once, so exact duplicate execution of a pinned transfer identity remains prevented. A retryable or transport-uncertain result gets at most one immediate retry with the same identity. If uncertainty remains, or the identity expires, Relay records a bounded best-effort outcome, abandons that recipient and continues. Definitive recipient rejection also cannot block later recipients. A plan-level failure such as a material fee change or insufficient remaining balance abandons the unpaid plan remainder. Any residual token balance may participate in a later fresh adjudication, which starts from current Ledger and history state and retains no exclusion for recipients paid by the abandoned payout. Cumulative recipient amounts across that abandonment boundary therefore are best-effort and are not guaranteed to reproduce the ideal pro-rata split of the original pot.

Individual SNS reward discrepancies are therefore tolerated as bounded best-effort outcomes. They are not converted into an indefinite recovery obligation, and one ambiguous reward must not halt subsequent reward distributions. This bounded-liveness policy is deliberate: exact pinned identities remain duplicate-safe while later fresh adjudications remain free to use the then-current economic state, and no result requires controller recovery.
...
The value-moving paths are designed around fixed transfer identities and bounded ambiguity handling:
...
- splitter state and compact reward-payout state use isolated stable memory so in-flight identities can survive a same-canister upgrade; and
...
For the canonical Relay, the following are the **legacy ICP Ledger account identifiers** (the 64-character hexadecimal addresses), rather than only the ICRC owner/subaccount notation. These are the Relay-owned ICP accounts used for immediate funding, Faucet-endowment staging, and the fixed splitter routes.
...
| Subaccount 1 | Jupiter Faucet endowment staging | `01` | `9fffa5e0762fd8be8e4c3078d4101926fb8d3c15aa3fa077b981ea779ded42ee` |
...
- [Jupiter Faucet](../faucet/README.md) — endowment and raw-ICP source used for perpetual Relay funding
