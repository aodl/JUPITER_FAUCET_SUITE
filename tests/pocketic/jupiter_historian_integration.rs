...
use ic_stable_structures::{
    ...
    storable::Bound,
    StableBTreeMap, StableCell, Storable, VectorMemory,
};
...
struct PublicStatus {
    ...
    commitment_index_fault: Option<CommitmentIndexFault>,
    route_index_fault: Option<String>,
}
...
#[derive(Clone, Debug, CandidType, Deserialize)]
struct FixtureRecentCommitment {
    canister_id: Principal,
    raw_icp_memo_text: Option<String>,
    tx_id: u64,
    timestamp_nanos: Option<u64>,
...
#[derive(Clone, Debug, CandidType, Deserialize)]
struct FixtureRecentNeuronCommitment {
    neuron_id: u64,
    memo_text: Option<String>,
    tx_id: u64,
    timestamp_nanos: Option<u64>,
...
#[derive(Clone, Debug, CandidType, Deserialize)]
struct FixtureInvalidCommitment {
    tx_id: u64,
    timestamp_nanos: Option<u64>,
    amount_e8s: u64,
    memo_text: String,
...
#[derive(Clone, Debug, CandidType, Deserialize)]
struct FixtureCommitmentIndexFault {
    observed_at_ts: u64,
    last_cursor_tx_id: Option<u64>,
    offending_tx_id: u64,
    message: String,
...
#[derive(Clone, Debug, CandidType, Deserialize)]
struct FixtureIcpXdrRateSnapshot {
    rate: u64,
    decimals: u32,
    timestamp: u64,
    fetched_at_ts: u64,
...
#[derive(Clone, Debug, CandidType, Deserialize)]
struct FixtureActiveSnsDiscovery {
...
#[derive(Clone, Debug, CandidType, Deserialize)]
struct FixtureActiveCyclesSweep {
    started_at_ts_nanos: u64,
...
#[derive(Clone, Debug, CandidType, Deserialize)]
struct FixtureActiveRouteSweep {
    started_at_ts_nanos: u64,
    next_index: u64,
}

#[derive(Clone, Debug, CandidType, Deserialize)]
struct FixtureDescendingIndexCatchUp {
    boundary_tx_id: u64,
    observed_head_tx_id: u64,
    next_start_tx_id: Option<u64>,
}

#[derive(Clone, Copy, Debug, CandidType, Deserialize)]
enum FixtureCommitmentIndexLeaseOwner {
    Scheduled,
    EndowmentRefresh,
}

#[derive(Clone, Debug, CandidType, Deserialize)]
struct FixtureStableRootState {
    config: FixtureStableConfig,
    last_indexed_staking_tx_id: Option<u64>,
    oldest_indexed_staking_tx_id: Option<u64>,
    staking_index_descending: Option<bool>,
    staking_backfill_complete: Option<bool>,
    ...
    last_completed_cycles_sweep_ts: u64,
    last_completed_route_sweep_ts: Option<u64>,
    active_cycles_sweep: Option<FixtureActiveCyclesSweep>,
    initial_cycles_probe_queue: Vec<Principal>,
    active_route_sweep: Option<FixtureActiveRouteSweep>,
    active_sns_discovery: Option<FixtureActiveSnsDiscovery>,
    main_lock_state_ts: Option<u64>,
    last_main_run_ts: u64,
    ...
    recent_commitments: Option<Vec<FixtureRecentCommitment>>,
    recent_under_threshold_commitments: Option<Vec<FixtureRecentCommitment>>,
    recent_neuron_commitments: Option<Vec<FixtureRecentNeuronCommitment>>,
    recent_under_threshold_neuron_commitments: Option<Vec<FixtureRecentNeuronCommitment>>,
    recent_invalid_commitments: Option<Vec<FixtureInvalidCommitment>>,
    recent_burns: Option<Vec<FixtureRecentBurn>>,
    last_index_run_ts: Option<u64>,
    commitment_index_fault: Option<FixtureCommitmentIndexFault>,
    icp_xdr_rate: Option<FixtureIcpXdrRateSnapshot>,
    last_icp_xdr_rate_attempt_ts: Option<u64>,
    last_icp_xdr_rate_error: Option<String>,
    active_staking_catch_up: Option<FixtureDescendingIndexCatchUp>,
    active_output_catch_up: Option<FixtureDescendingIndexCatchUp>,
    active_rewards_catch_up: Option<FixtureDescendingIndexCatchUp>,
    commitment_index_lock_expires_at_ts: Option<u64>,
    commitment_index_lock_generation: Option<u64>,
    commitment_index_lock_owner: Option<FixtureCommitmentIndexLeaseOwner>,
...
#[derive(Clone, Debug, CandidType, Deserialize)]
enum FixtureVersionedStableState {
    Uninitialized,
    Current(FixtureStableRootState),
}

impl Storable for FixtureVersionedStableState {
    fn to_bytes(&self) -> Cow<'_, [u8]> {
        Cow::Owned(encode_one(self).expect("encode Historian root fixture"))
    }

    fn into_bytes(self) -> Vec<u8> {
        encode_one(self).expect("encode Historian root fixture")
    }

    fn from_bytes(bytes: Cow<'_, [u8]>) -> Self {
        decode_one(bytes.as_ref()).expect("decode Historian root fixture")
    }

    const BOUND: Bound = Bound::Unbounded;
}

fn rewrite_historian_root_fixture(
    pic: &PocketIc,
    historian: Principal,
    update: impl FnOnce(&mut FixtureStableRootState),
) {
    let stable_memory = Rc::new(RefCell::new(pic.get_stable_memory(historian)));
    let manager = MemoryManager::init(stable_memory.clone());
    let mut cell = StableCell::<FixtureVersionedStableState, _>::init(
        manager.get(MemoryId::new(0)),
        FixtureVersionedStableState::Uninitialized,
    );
    let mut state = cell.get().clone();
    let FixtureVersionedStableState::Current(root) = &mut state else {
        panic!("expected initialized Historian root fixture");
    };
    update(root);
    cell.set(state);
    drop(cell);
    ...
    pic.set_stable_memory(historian, bytes, BlobCompression::NoCompression);
}
...
#[test]
#[ignore]
fn legacy_ascending_route_state_is_repaired_once_and_rebuilt_from_index_history() -> Result<()> {
    require_ignored_flag()?;
    let h = Harness::new_with_scan_interval(false, 3_600)?;
    let staking_id = h.staking_identifier()?;
    let commitment_target = Principal::from_slice(&[73]);
    let _: u64 = update_bytes(
        &h.pic,
        h.index,
        Principal::anonymous(),
        ...
        encode_args((
            staking_id,
            100_000_000_u64,
            Some(commitment_target.to_text().into_bytes()),
        ))?,
    )?;
    let _: () = update_noargs(
        &h.pic,
    ...
    assert_eq!(counts_before.qualifying_commitment_count, 1);
    let summary_args = GetCommitmentRouteSummariesArgs {
        routes: vec![CommitmentRoute::CyclesTopUp {
            canister_id: commitment_target,
        }],
    };
    let summaries_before: GetCommitmentRouteSummariesResponse = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_commitment_route_summaries",
    ...
    let tracked_before: ListCanistersResponse = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "list_canisters",
    ...
    let setup_entries_before = relay_setup_entry_count(&h.pic, h.historian);
    let config: DebugRouteConfig = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "debug_config",
    ...
    let source_id = account_identifier_text(
        config.output_source_account.owner,
        config.output_source_account.subaccount,
    );
    let output_id = account_identifier_text(
        config.output_account.owner,
        config.output_account.subaccount,
    );
    let rewards_id = account_identifier_text(
        config.rewards_account.owner,
        config.rewards_account.subaccount,
    );

    for (from, to, amount) in [
        (source_id.clone(), output_id.clone(), 11_u64),
        (
            "unrelated-output-source".to_string(),
            output_id.clone(),
            10_000,
        ),
        (source_id.clone(), output_id.clone(), 13),
        (source_id.clone(), rewards_id.clone(), 17),
        (
            "unrelated-rewards-source".to_string(),
            rewards_id.clone(),
            20_000,
        ),
        (source_id.clone(), rewards_id.clone(), 19),
    ] {
        let _: u64 = update_bytes(
            &h.pic,
            h.index,
            ...
            encode_args((from, to, amount, Option::<Vec<u8>>::None))?,
        )?;
    }
    ...
    let historian_controller = h
        ...
        .unwrap_or(Principal::anonymous());
    support::governance::stop_canister_as(&h.pic, h.historian, historian_controller)?;
    rewrite_historian_root_fixture(&h.pic, h.historian, |root| {
        root.total_output_e8s = Some(91_000_000);
        root.last_indexed_output_tx_id = Some(91);
        root.oldest_indexed_output_tx_id = Some(9);
        root.output_route_index_descending = Some(false);
        root.output_route_backfill_complete = Some(true);
        root.active_output_catch_up = Some(FixtureDescendingIndexCatchUp {
            boundary_tx_id: 91,
            observed_head_tx_id: 99,
            next_start_tx_id: Some(95),
        });
        root.total_rewards_e8s = Some(82_000_000);
        root.last_indexed_rewards_tx_id = Some(82);
        root.oldest_indexed_rewards_tx_id = Some(8);
        root.rewards_route_index_descending = Some(false);
        root.rewards_route_backfill_complete = Some(true);
        root.active_rewards_catch_up = Some(FixtureDescendingIndexCatchUp {
            boundary_tx_id: 82,
            observed_head_tx_id: 89,
            next_start_tx_id: Some(85),
        });
        root.active_route_sweep = Some(FixtureActiveRouteSweep {
            started_at_ts_nanos: 123_000_000_000,
            next_index: 1,
        });
        root.last_completed_route_sweep_ts = Some(123);
    });
    h.pic
        .upgrade_canister(
            h.historian,
        ...
        .map_err(|error| anyhow!("upgrade legacy-route Historian fixture: {error:?}"))?;
    support::governance::start_canister_as(&h.pic, h.historian, historian_controller)?;

    let repaired: DebugState = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "debug_state",
    ...
    assert_eq!(repaired.last_indexed_output_tx_id, None);
    assert_eq!(repaired.oldest_indexed_output_tx_id, None);
    assert_eq!(repaired.output_route_index_descending, Some(true));
    assert_eq!(repaired.output_route_backfill_complete, Some(false));
    assert_eq!(repaired.last_indexed_rewards_tx_id, None);
    assert_eq!(repaired.oldest_indexed_rewards_tx_id, None);
    assert_eq!(repaired.rewards_route_index_descending, Some(true));
    assert_eq!(repaired.rewards_route_backfill_complete, Some(false));
    assert!(!repaired.active_route_sweep_present);
    assert_eq!(repaired.last_completed_route_sweep_ts, Some(0));
    let repaired_counts: PublicCounts = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_public_counts",
    ...
    assert_eq!(repaired_counts.total_output_e8s, 0);
    assert_eq!(repaired_counts.total_rewards_e8s, 0);
    assert_eq!(
        repaired_counts.qualifying_commitment_count,
        counts_before.qualifying_commitment_count
    );
    let repaired_status: PublicStatus = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_public_status",
    ...
    assert_eq!(
        repaired_status.route_index_fault.as_deref(),
        Some("output and rewards historical route backfills are incomplete")
    );
    let summaries_after_repair: GetCommitmentRouteSummariesResponse = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_commitment_route_summaries",
    ...
    assert_eq!(summaries_after_repair.items, summaries_before.items);
    assert_eq!(
        summaries_after_repair.complete_from_genesis,
        summaries_before.complete_from_genesis
    );
    let tracked_after_repair: ListCanistersResponse = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "list_canisters",
    ...
    assert_eq!(tracked_after_repair.items, tracked_before.items);
    assert_eq!(
        relay_setup_entry_count(&h.pic, h.historian),
        setup_entries_before,
        "memory 26 must be untouched by the route repair"
    );
    ...
    let output_complete: PublicCounts = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_public_counts",
    ...
    assert_eq!(output_complete.total_output_e8s, 24);
    assert_eq!(output_complete.total_rewards_e8s, 0);
    let output_status: PublicStatus = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_public_status",
    ...
    assert_eq!(
        output_status.route_index_fault.as_deref(),
    ...
    let rebuilt: PublicCounts = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_public_counts",
    ...
    assert_eq!(rebuilt.total_output_e8s, 24);
    assert_eq!(rebuilt.total_rewards_e8s, 36);
    let healthy: PublicStatus = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_public_status",
    ...
    assert!(healthy.route_index_fault.is_none());

    for (to, amount) in [(output_id, 23_u64), (rewards_id, 29_u64)] {
        let _: u64 = update_bytes(
            &h.pic,
            h.index,
            Principal::anonymous(),
            "debug_append_transfer_from",
            encode_args((source_id.clone(), to, amount, Option::<Vec<u8>>::None))?,
        )?;
    }
    for _ in 0..2 {
        let _: () = update_noargs(
    ...
    let caught_up: PublicCounts = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_public_counts",
    ...
    assert_eq!(caught_up.total_output_e8s, 47);
    assert_eq!(caught_up.total_rewards_e8s, 65);
    for _ in 0..2 {
        let _: () = update_noargs(
            &h.pic,
            h.historian,
    ...
    let no_duplicates: PublicCounts = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_public_counts",
    ...
    assert_eq!(no_duplicates.total_output_e8s, 47);
    assert_eq!(no_duplicates.total_rewards_e8s, 65);

    h.pic.advance_time(Duration::from_secs(61));
    tick_n(&h.pic, 5);
    upgrade_historian_without_config_changes(&h.pic, h.historian)?;
    let after_second_upgrade: PublicCounts = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_public_counts",
    ...
    assert_eq!(after_second_upgrade.total_output_e8s, 47);
    assert_eq!(after_second_upgrade.total_rewards_e8s, 65);
    assert_eq!(
        after_second_upgrade.qualifying_commitment_count,
        counts_before.qualifying_commitment_count
    );
    let final_status: PublicStatus = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_public_status",
    ...
    assert!(final_status.route_index_fault.is_none());
    assert_eq!(
        relay_setup_entry_count(&h.pic, h.historian),
        setup_entries_before
    );
...
fn historian_route_indexing_with_real_icp_index_counts_descending_route_pages_without_stalling(
    ...
    let staking_id = account_identifier_text(staking_account.owner, staking_account.subaccount);
    let commitment_target = Principal::from_slice(&[41]);
    for (memo, amount) in [
        (commitment_target.to_text().into_bytes(), 110_000_000_u64),
        (
            format!("{}.", commitment_target.to_text()).into_bytes(),
            120_000_000_u64,
        ),
        (b"100.".to_vec(), 130_000_000_u64),
    ] {
        icrc1_transfer(
            &pic,
            ledger,
    ...
    wait_for_index_transactions(&pic, index, &staking_id, 3)?;
    ...
    let route_args = GetCommitmentRouteSummariesArgs {
        routes: vec![
            CommitmentRoute::CyclesTopUp {
                canister_id: commitment_target,
            },
            CommitmentRoute::RawIcp {
                destination_canister_id: commitment_target,
                memo: Vec::new(),
            },
...
fn historian_commitment_route_rollups_are_exact_lifetime_and_upgrade_stable() -> Result<()> {
    ...
    let canister_c = Principal::from_slice(&[43]);
    let staking_id = h.staking_identifier()?;
    ...
    let _: u64 = update_bytes(
        ...
        encode_args((
            ...
            500_u64,
            230_000_000_u64,
            Some(b"not-a-valid-route".to_vec()),
        ))?,
    )?;
