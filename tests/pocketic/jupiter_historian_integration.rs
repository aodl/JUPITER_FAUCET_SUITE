...
impl Harness {
    ...
    fn skip_install_main_tick(&self) -> Result<DebugState> {
        let now_secs = (self.pic.get_time().as_nanos_since_unix_epoch() / 1_000_000_000) as u64;
        let _: () = update_one(
            &self.pic,
            self.historian,
            Principal::anonymous(),
...
fn event_horizon_poke_filters_ids_and_checks_one_bounded_page() -> Result<()> {
    ...
    assert_eq!(counts.qualifying_commitment_count, 500);
...
fn event_horizon_poke_rejects_ingress_and_untrusted_canisters() -> Result<()> {
    ...
    let before = h.pic.get_stable_memory(h.historian);
    let calls_before = debug_index_calls(&h)?.len();
    for caller in [
        Principal::anonymous(),
    ...
    let rejection: Result<(), String> = update_one(
        &h.pic,
        attacker,
        Principal::anonymous(),
        "debug_poke",
...
fn event_horizon_poke_trailing_check_and_hourly_poll_remain_fallbacks() -> Result<()> {
    ...
    tick_n(&h.pic, 8);
    let after_trailing_check: PublicCounts = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "get_public_counts",
    ...
    assert_eq!(after_trailing_check.qualifying_commitment_count, 1);
    let indexed: EndowmentTransactionStatusResponse = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
    ...
    assert_eq!(indexed.status, ExpectedEndowmentStatus::KnownIndexed);
    let after: DebugState = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
    ...
    assert_eq!(after.last_main_run_ts, initial.last_main_run_ts);
    let _: u64 = update_bytes(
        &h.pic,
        h.index,
        Principal::anonymous(),
    ...
    assert_eq!(after_coalesced.qualifying_commitment_count, 2);
    let _: u64 = update_bytes(
        &h.pic,
        h.index,
        Principal::anonymous(),
...
fn real_icp_index_lag_is_resolved_by_trailing_check_before_hourly_scan() -> Result<()> {
    ...
    pic.install_canister(historian, historian_wasm()?, encode_one(init)?, None);
    let initial: DebugState =
        query_one(&pic, historian, Principal::anonymous(), "debug_state", ())?;
    let started_at = pic.get_time().as_nanos_since_unix_epoch() / 1_000_000_000;
    let _: () = update_one(
        &pic,
        historian,
        Principal::anonymous(),
        "debug_set_main_lock_expires_at_ts",
        Some(started_at as u64 + 3_600),
    )?;
    ...
    assert!(
        index_account_transactions(&pic, index, staking_id.clone(), None, 10)?
            .transactions
            .iter()
            .all(|tx| tx.id != tx_id),
    ...
    let indexed_page = wait_for_index_transactions(&pic, index, &staking_id, 1)?;
    assert_eq!(indexed_page.transactions[0].id, tx_id);
    let before_trailing_check: EndowmentTransactionStatusResponse = query_one(
        &pic,
        historian,
        Principal::anonymous(),
        "get_endowment_transaction_status",
    ...
    let routes: GetCommitmentRouteSummariesResponse = query_one(
        &pic,
        historian,
        Principal::anonymous(),
    ...
    assert_eq!(routes.items[0].qualifying_commitment_count, 1);
    assert_eq!(routes.items[0].total_qualifying_committed_e8s, 100_000_000);
    let after: DebugState = query_one(&pic, historian, Principal::anonymous(), "debug_state", ())?;
    assert_eq!(after.last_main_run_ts, initial.last_main_run_ts);
    assert!(pic.get_time().as_nanos_since_unix_epoch() / 1_000_000_000 - started_at < 3_600);
    Ok(())
}
...
fn event_horizon_trailing_check_follows_the_most_recent_rapid_poke() -> Result<()> {
    ...
    tick_n(&h.pic, 8);
    assert_eq!(debug_index_calls(&h)?.len(), baseline + 1);
    h.pic.advance_time(Duration::from_secs(10));
    tick_n(&h.pic, 8);
    assert_eq!(debug_index_calls(&h)?.len(), baseline + 2);
    h.pic.advance_time(Duration::from_secs(30));
    tick_n(&h.pic, 8);
    assert_eq!(debug_index_calls(&h)?.len(), baseline + 2);
    Ok(())
}
...
#[test]
#[ignore]
fn legacy_poke_lease_root_restores_before_post_upgrade_reset() -> Result<()> {
    let h = Harness::new_with_scan_interval(false, 3_600)?;
    ...
    let _: () = update_noargs(
        ...
        h.historian,
        Principal::anonymous(),
        "debug_seed_legacy_poke_lease",
    )?;
    let seeded: DebugState = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "debug_state",
    ...
    assert!(seeded.commitment_index_lock_expires_at_ts.unwrap_or(0) > 0);
    assert!(seeded.poke_next_allowed_ts > 0);
    assert_ne!(h.pic.get_stable_memory(h.historian), stable_before);
    let controller = h
        .pic
    ...
    support::governance::stop_canister_as(&h.pic, h.historian, controller)?;
    h.pic
        .upgrade_canister(
            h.historian,
            historian_wasm()?,
        ...
        .map_err(|error| anyhow!("upgrade legacy-lease Historian: {error:?}"))?;
    support::governance::start_canister_as(&h.pic, h.historian, controller)?;
    let restored: DebugState = query_one(
        &h.pic,
        h.historian,
        Principal::anonymous(),
        "debug_state",
    ...
    assert_eq!(restored.commitment_index_lock_expires_at_ts, Some(0));
    assert_eq!(restored.poke_next_allowed_ts, seeded.poke_next_allowed_ts);
    Ok(())
}
...
fn pending_genesis_backfill_survives_an_actual_historian_upgrade() -> Result<()> {
    ...
    assert_eq!(first.qualifying_commitment_count, 500);
