...
This repository includes build, installation, upgrade, and recovery material so the canonical deployment can be operated and independently verified, and so contributors can test and extend the system locally. Application developers can instead begin at the integration layer: choose a supported endowment route or create a self-service Relay through the deployed protocol.
...
| Turn long-term ICP into recurring funding | Usually generic | Faucet endowment |
...
The boundary is about responsibility, not about forcing one deployment pattern. A single-canister project may use only a direct Faucet endowment. A multi-canister project may use a Relay with or without Faucet. Another project may keep its existing treasury policy and use Jupiter only for one funding route.

## Composition model

Jupiter Faucet is a set of composable primitives rather than a monolithic service:

1. **Direct Faucet endowment.** A qualifying endowment participates in recurring Faucet payouts. Its exact route selects cycles top-ups to a canister, raw ICP to a canister account with exact memo bytes, or supported NNS neuron staking-account funding. The [Faucet README](../../canisters/faucet/README.md) defines the exact endowment, memo, payout, fee, and best-effort delivery rules.
...
3. **Faucet and Relay together.** A Faucet endowment can route recurring raw ICP to a Relay. Faucet supplies the long-term funding stream; Relay decides how the currently available ICP should serve the application's changing infrastructure needs.
4. **Immediate and future funding.** Relay's intrinsic splitter accounts divide a deposit between its immediately spendable default account and subaccount 1, which builds a Faucet endowment that can feed the Relay again. The supported fixed splits are documented in [Funding a Relay](../../canisters/relay/README.md#funding-a-relay).
...
A project makes a Faucet endowment directly targeting its canister. The project keeps its application logic but does not need to operate its own recurring maturity-harvesting and cycles top-up scheduler for that route.
...
A project combines Faucet and Relay for a long-term funding path. Relay splitter funding can leave one portion available for current infrastructure while directing another portion toward a Faucet endowment that may produce future funding.
...
- **Public operational evidence.** Components publish runtime configuration and status through documented logs and read surfaces. Historian maintains bounded endowment, output, rewards, and cycles views; full transfer authority remains with the relevant ledgers and indexes. See the [Historian README](../../canisters/historian/README.md).
...
- **Explicit recovery posture.** Faucet and Disburser separate public observation from controller authority: healthy autonomous control is self only, while sustained or forced failure adds Lifeline under component-specific health rules. Generic blackhole and SNS observation routes remain compatibility mechanisms for external targets. Controllerless self-service Relays use a distinct immutable posture.
...
The broadest high-level view is the suite overview below. It shows the relationship between a long-term ICP endowment, recurring maturity and output, direct Faucet routes, Relay-managed cycles allocation, observability, and the intended relationship with the pre-launch IO project.
