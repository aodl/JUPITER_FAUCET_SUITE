...
Historian, Faucet, Disburser, and Relay accept permissioned Event Horizon hints where incoming ICP can accelerate their existing work; normal polling remains authoritative. SNS Rewards and Lifeline do not use Event Horizon because their periodic work is not an incoming-ICP detection problem.
...
- Relay, rather than Historian, owns endowment attribution and reward-token forwarding. Historian has no registry, eligibility, Ledger-discovery, or settlement role in this flow.

Protocol-native status visibility is the preferred observability mechanism: `public` exposes the management canister's status response to every caller without granting controller authority. Jupiter protocol canisters therefore do not require the Fiduciary blackhole as a controller for status or cycles observation. Recognized blackhole and SNS status routes remain compatibility fallbacks for third-party canisters and for observing the recognized blackholes themselves. Canonical and self-service Relays use Auto probing; manually configured Relays may still select FixedBlackhole. Healthy autonomous Faucet/Disburser control is self only, and their explicit failure policy adds Lifeline for recovery. Self-service Relays instead become immutable with zero controllers while retaining public logs and public status.
