...
- [`canisters/relay`](../../canisters/relay) receives suite-funding ICP from the faucet, samples cycles balances for managed canisters, tops up recent burn plus headroom, routes usable surplus, and attributes completed subaccount-1 commitments before forwarding the current SNS reward-token pot to one eligible winner. For credits from its intrinsic splitters 10–90, it reconstructs the original AccountIdentifiers from bounded ICP Index history and weights only the actual net SubaccountOne leg.
