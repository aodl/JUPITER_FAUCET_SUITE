...
`max_transfers_per_tick` is `target_count + recipient_count + 1`; the final slot covers Relay self in the effective managed set. The maximum self-service configuration therefore uses 26. The five-recipient limit belongs to Historian self-service policy, not Relay runtime.
