...
Exact live evidence captured on 2026-09-10 shows this one-hop state bridge is still required. Faucet module `07a01016f88ab0e039378b49c03a33da31b82075668437fba09bb84573fa91af` publishes `blackhole_armed=none` and Disburser module `af801944f58dba1f9357f91cbf1bd04a30f281ed1254268eb2b0a1e672b99af2` publishes `blackhole_armed=false`; neither publishes `autonomous_rescue_armed`. Both live modules therefore predate controller-separation commit `b17a8f9ef127a547f6792e8bd42bd2a54af819c2`. A second public check on 2026-09-11 confirmed the Faucet still runs module `07a01016...fa91af`, and its latest recurring `CONFIG` publishes `canonical_relay_canister_id=u2qkp-aqaaa-aaaar-qb7ea-cai`. That CONFIG marker was introduced in commit `0c9a7a26765eb72a606593199caf141521a73126`, the same source change that replaced the stable `RemainderToSelf`/`remainder_to_self_e8s` representation with `RemainderToRelay`/`remainder_to_relay_e8s`. The live Faucet therefore writes the post-Relay-remainder stable shape, so the retained old-controller-to-current-controller bridge is sufficient and no distinct pre-Relay decoder belongs in the temporary bridge build. After each module has upgraded successfully and current `CONFIG` plus durable-state restoration are verified, apply the prepared cleanup patch immediately; the final pre-launch source must not retain this migration.
