...
Memory 24 is the earlier retired pre-launch setup-job area, memory 25 is the retired target-set-hash setup map, and memory 26 is the definitive full-configuration setup map. A direct upgrade from a version predating memory 25 first performs the earlier retired-registry/memory-24 validation and initializes memory 25 empty. The memory-26 cutover then requires memory 25 to contain zero entries. Any memory-25 entry fails closed without being cleared, decoded into the new identity, or migrated. Memory 26 is initialized empty only after that check, and old target-only setup hashes and addresses are not consulted or reused. If either upgrade gate fails, do not proceed; restore the retained snapshot through the rehearsed rollback procedure. On later upgrades, `Reserved`/`ProbingTargets` entries are removed, later `Creating` phases become `ManualRecoveryRequired`, and existing `Active`/`ManualRecoveryRequired` entries in memory 26 are preserved. Retain the snapshot until controlled acceptance is complete. Historian does not upgrade or reconstruct blackholed children. Deploy the final frontend only after backend verification.
