...
The funding cursor and staking scan are separate in these tests: the funding cursor selects the next payout-account funding tranche, while staking-account history remains replayable so older commitments keep contributing. Legacy-upgrade scenarios without an exact cursor seed may have an imperfect first strict round because the oldest observed unprocessed funding marker is used; subsequent rounds self-align once the funding cursor advances. For the current single-participant production deployment, operations can reduce that first-round risk by avoiding fresh commitments inside the recognition-delay window. Multi-user deployments should prefer exact cursor/config alignment before strict tranche rollout.
