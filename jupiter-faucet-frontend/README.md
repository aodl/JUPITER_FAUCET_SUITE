...
- the displayed historian history tables are intentionally bounded views and now also show the historian canister's current allocated memory footprint; stale memo-only target-canister entries that never qualified are pruned during historian normalization, while the retained qualifying/SNS-backed registry remains durable, and full transfer history still lives on the ICP ledger and its archive canisters
