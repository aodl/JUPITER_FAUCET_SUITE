...
- the displayed historian history tables are intentionally bounded views and now also show the historian canister's current allocated memory footprint; the tracked canister/principal registry is not pruned, and full transfer history still lives on the ICP ledger and its archive canisters
