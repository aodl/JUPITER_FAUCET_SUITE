...
That ordering also matters for the faucet's round-accounting fairness model. Newly added stake may become visible in the neuron's live stake before its proportional base maturity has fully reached the faucet payout account. The faucet now addresses that directly with a round-effective denominator: it carries a round-start staking snapshot forward, clamps the completed round by tx id, and applies a conservative stake-recognition delay before weighting valid in-round contributions into the denominator. The disburser-side PocketIC suite still exercises the full stake -> maturity -> disburser -> faucet path, but the unfair first-round dilution condition is now mitigated in faucet logic rather than merely observed.

## PocketIC test note on maturity variability

The PocketIC maturity/payout tests in this repository do **not** assume that a constant neuron stake implies a constant round payout. In the harness, nearby rounds can still differ because the reward environment itself moves: proposal settlement timing, reward rollover, total voting power, and related governance-side factors can change how much maturity becomes stageable in a given synthetic round.

For that reason, the most informative protocol-level tests now:

- use a large background whale neuron to stabilize total voting power and reduce sensitivity to unrelated NNS background neurons
- treat the whale's **final cached stake** as the relevant stabilization parameter; the helper computes the target whale size up front and seeds the neuron directly with that amount
- warm up and explicitly drain any pre-existing maturity before establishing a baseline
- keep canonical post-mitigation economics tests that check very late valid and very late invalid top-ups do **not** reduce the existing beneficiary's affected-round payout under weighted round accounting

The detailed caveats and helper rationale live in [`../xtask/README.md`](../xtask/README.md) and in comments next to the PocketIC reward helpers.
