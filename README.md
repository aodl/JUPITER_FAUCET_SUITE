...
Routine no-config-change production upgrades pass no args for Disburser, Faucet, and Historian. Existing production Historian must be upgraded in place; reinstall destroys its stable histories, tracking metadata, Relay registrations, setup/recovery jobs, cursors, and aggregates, and is prohibited for the existing production canister. Checked-in install argument files are fresh install/reinstall `InitArgs`, not routine upgrade inputs for those stateful canisters; `canisters/historian/mainnet-install-args.did` is for a brand-new Historian installation only. Config-changing upgrades use temporary canister-specific `Option<UpgradeArgs>` files where supported. Relay is replacement-style: every Relay install, reinstall, or upgrade passes full `InitArgs`, initializes fresh heap state, and resets operational state. Relay upgrades are non-resumable; avoid upgrading during active Relay work where practical, and reconcile manually if an operation is interrupted. Its first successful post-upgrade tick is expected to be `BaselineOnly`.
